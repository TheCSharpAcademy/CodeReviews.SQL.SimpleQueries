...
SELECT * FROM dbo.tblCategory WHERE CategoryID > (SELECT COUNT(*) FROM dbo.tblCategory) - 3 ORDER BY CategoryName;
...
SELECT TOP 5 EventName AS Event, EventDetails AS Details FROM dbo.tblEvent ORDER BY EventDate;
...
SELECT TOP 2 EventID, EventName, EventDetails, EventDate FROM dbo.tblEvent
UNION
SELECT TOP 2 EventID, EventName, EventDetails, EventDate FROM dbo.tblEvent WHERE EventID > (SELECT COUNT(*) FROM dbo.tblEvent) ORDER BY EventDate;
